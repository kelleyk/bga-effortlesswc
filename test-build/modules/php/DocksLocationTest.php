...
final class DocksLocationTest extends \Effortless\Test\IntegrationTestCase
{
  ...
  {
    print('******* XXX: inheritance:');
    printClassInheritance('DocksLocationTest');
...
function printClassInheritance(string $className, int $level = 0): void {
  ...
  echo "{$indent}{$className}\n";

  $parentClass = get_parent_class($className);
  if ($parentClass !== false) {
    printClassInheritance($parentClass, $level + 1);
  }

  $interfaces = class_implements($className);
  foreach ($interfaces as $interface) {
    echo "{$indent}  implements {$interface}\n";
  }

  $traits = class_uses($className);
  foreach ($traits as $trait) {
    echo "{$indent}  uses {$trait}\n";
  }
}
